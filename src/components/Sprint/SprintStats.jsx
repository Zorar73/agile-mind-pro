...
function SprintStats({ sprint, tasks = [] }) {
  ...
  const StatCard = ({ icon: Icon, value, label, color }) => (
    <Card sx={{ bgcolor: isDark ? 'background.paper' : 'white', borderRadius: 2, border: isDark ? '1px solid' : 'none', borderColor: 'divider', boxShadow: isDark ? 'none' : 1, flex: 1 }}>
      <CardContent sx={{ py: 1.5, px: 2, '&:last-child': { pb: 1.5 } }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Box>
            <Typography variant="h6" fontWeight={700} color={color} sx={{ lineHeight: 1 }}>{value}</Typography>
  ...
  return (
    <Box>
      ...
      {/* Прогресс */}
      <Box sx={{ bgcolor: isDark ? 'rgba(255,255,255,0.03)' : 'grey.50', borderRadius: 2, p: 1.5 }}>
